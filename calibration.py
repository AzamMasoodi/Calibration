...
class LisemKOptimizer:
    def __init__(self, run_file,output_file, obs_file, lisem_path):
        self.run_file = run_file
        self.obs_file = obs_file
        # self.output_file = output_file
        self.lisem_path = lisem_path
        
    def opt_k(self, min_k, max_k, num_steps=5):
        """
    ...
    def run_opt_round(self, k_values: list, round_no: int):
            ...
            for run_no, k in enumerate(k_values):
                ...
                print(f'round = {round_no}, run = {run_no}/{len(k_values)}, k = {k_values[run_no]}')
            return results
    ...
    def get_value_from_runfile(self, variablename):
        ...
        if not m:
            raise ValueError(f'{variablename} not in lisem runfile {self.run_file}')
        return m.group(1)
    ...
    def open_lisem(self, k):
        ...
        """
        #Open ksat raster - can't be an absolut path. Change to relative path with variable name
    ...
    def filterdata(self):
        ...
        """ 
        sim_file = self.get_value_from_runfile('Result Directory') + '/totalseries.csv'
...
if __name__ == '__main__':
    ...
    run_path = "C:/Masoodi/test/run1/run1.run"
    k_path="C:/Masoodi/test/map1/ksat1.map"
    sim_file = 'C:/Masoodi/test/res1/totalseries.csv'
    # Specify the observation CSV file path
    obs_file='C:/Masoodi/test/obs1/obs1.csv'
    # Specify the filtered CSV file path
    output_file='C:/Masoodi/test/filtered_data.csv'
    # Specify the input CSV file path
    # opt_k(10, 60, num_steps=5)           
    opt = LisemKOptimizer(run_path,output_file, obs_file, lisem_path)
    print(opt.get_value_from_runfile('Map Directory'))
